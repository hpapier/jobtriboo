...
const RoomItem = ({ userState, data, open, t, logInfo = null }) => {
  ...
  return (
    <div className={`room-item-root${!lastMessageInfo.readed && lastMessageInfo.from !== logInfo.userId ? `` : ` -room-inactive`}`} onClick={open}>
      { userState === 'candidate' || ((candidate.picture === '') ? <div className='room-item-root-picture'></div> : <img src={`http://localhost:3001${candidate.picture}`} className='room-item-root-picture' alt='candidate-picture' />) }
